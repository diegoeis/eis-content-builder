...
> **Synopsis.** Brainstorm angles or discuss a topic before writing. Two modes: open ideation (scouts trusted sources, returns 5-8 angles annotated against your opinion-map) and topic discussion (Socratic loop → locked thesis → ready-to-paste `/writer-create` command). Never invokes `/writer-create` directly.
