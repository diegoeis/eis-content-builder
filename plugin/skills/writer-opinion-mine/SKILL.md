...
> **Synopsis.** Socratic interview to build or deepen your opinion-map. Four modes: topic (extract/firm up one position), tensions (surface contradictions between positions), refusals (map non-positions), authorities (populate how you read key thinkers). Max 15 min per session, 1 topic per session.
